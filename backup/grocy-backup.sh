...
if ./dbxcli account; then
  mkdir -p backup
  sudo docker exec grocy_1 apk add sqlite
  sudo docker exec grocy_1 sqlite3 /www/data/grocy.db ".backup 'backup_grocy.db'"
  sudo docker cp grocy_1:/www/public/backup_grocy.db backup/grocy.db
